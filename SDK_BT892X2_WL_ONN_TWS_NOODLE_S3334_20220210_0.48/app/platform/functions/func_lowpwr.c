...
//#endif
    ...
    RTCCON3 = rtccon3;
//#if WL_SMART_HOUSE_EN			//充电仓发完关机码自动开机
//	RTCCON3 |= BIT(5);     // rtc3 bit 5 1 上5V开机，rtc3 bit 12 至 0
//#endif
    LPMCON |= BIT(0);
    asm("nop");asm("nop");asm("nop");
    while (1);
...
void func_pwroff(int pwroff_tone_en)
{
    printf("%s\n", __func__);
		...
		if(sys_cb.from_box_power_on == 0)		//仓外关机
		{
			sys_cb.pwr_flag = 1;
			param_inbox_write(1);
			printf("write pwrfalg === ==  1\n");
		}
		else		//仓内超时关机
		{
			sys_cb.pwr_flag = 0;
			param_inbox_write(0);		//仓内关机
			printf("write pwrfalg === ==  0\n");
		}
#endif
#if VUSB_SMART_VBAT_HOUSE_EN
	...
	if(pwroff_tone_en == 0)
	{
		sys_cb.charge_ok_led_flag = 1;
		static u32 connt_tick = 0;
		delay_ms(5);
		LED_PWR_SET_ON();
		connt_tick = tick_get();
		while (!tick_check_expire(connt_tick, 1500)) {	   //延时30ms再发
			delay_5ms(1);
			WDT_CLR();
		}
		connt_tick = tick_get();
    ...
    if (SOFT_POWER_ON_OFF) {
        ...
        dac_power_off();                    //dac power down
        if (CHARGE_DC_IN()) {
#if FAST_PWROFF_INBOX					//仓内关机快速pwrdown 1
			if(0 == sys_cb.pwr_off_timeout_cnt){
			}else{
            if (power_off_check()) {
                return;
            }
