...
void sco_wait_sync()
{
    bsp_sys_mute();
    if (!bt_tws_is_slave()) {
        bt_tws_user_key(TWS_USER_KEY_TWS_SCO_WAIT);
        bt_tws_set_syncfunc(BSP_TWS_SYNC_SCO, 200, 0);
    }
    sys_cb.sco_wait_timeout = 2000;
}

WEAK bool bt_tws_is_sniff()
{
    return false;
}
#endif
...
void sco_audio_init(void)
{
    ...
    if (xcfg_cb.bt_sndp_dm_en && xcfg_cb.micl_en && xcfg_cb.micr_en) {
        ...
        dmic_cb.windEQ_en    = 1;
        dmic_cb.fade_en      = BT_SNDP_DMIC_FADE_EN;
        dmic_cb.fade_time    = BT_SNDP_DMIC_FADE_TIME;
        //默认关闭以下参数，需要可手动打开
...
//    dmns_cb.lowside_corr        = 130;
//	dmns_cb.fast_convergence_en = 1;
//	dmns_cb.filter_constraint   = 15;
//	dmns_cb.param_printf        = 1;
	dmns_cb.trumpet_en  = BT_TRUMPET_DENOISE_EN;
	...
	sysclk = BT_NLMS_AEC_EN ? SYS_120M : SYS_80M;
#endif

#if BT_SCO_DNN_EN
    nr->type = (nr->type & ~0x07) | NR_CFG_TYPE_DNN;
	nr->mode = BT_SCO_DNN_MODE;
    nr->level = BT_SCO_DNN_LEVEL;
    sysclk = BT_NLMS_AEC_EN ? SYS_160M : SYS_120M;
#endif
    ...
    if (xcfg_cb.huart_en) {
    #if BT_SNDP_DUMP_EN || BT_AINS2_DUMP_EN || BT_DMNS_DUMP_EN || BT_DNN_DUMP_EN
...
static void sfunc_bt_call_process(void)
{
    ...
    if (xcfg_cb.bt_tswi_sco_en && bt_tws_need_switch(0)) {
		if(!((!bt_tws_is_slave()) && ((!sys_cb.loc_ear_sta && sys_cb.rem_ear_sta) || (!sys_cb.rem_ear_sta && sys_cb.loc_ear_sta))))
		{
			bt_tws_switch();
		}
...
static void sfunc_bt_call_enter(void)
{
    ...
    if(sys_cb.incall_flag == INCALL_FLAG_FADE) {
        ...
        //dac_fade_out();
		bsp_sys_mute();
        sys_cb.dac_fade_sta = 1;
        sys_cb.dac_fade_tick = tick_get();
#endif
