...
{
#if IOS_INCOMING_RING
	if(bt_is_ios_device()) return;
#endif
    if(bt_tws_is_connected()){
        delay_ticks = 5;
    }else{
...
static void sfunc_bt_ring_process(void)
{
	...
	if(bt_get_disp_status() == BT_STA_INCOMING && CHARGE_INBOX())
	{
...
#if RING_NO_RESPON_MSG
    func_bt_sub_process();			//修改来电时，不会触发入耳检测的问题
#endif
}
