...
void bsp_vhouse_pair_ack(vh_packet_t *packet)
{
    ...
    if((channel != VHOUSE_LEFT_CHANNEL)||vhouse_cb.cmd3_rx_flag){
        ...
        if(!bt_get_qr_addr(packet->buf+7)){
...
void bsp_vhouse_get_tws_btaddr_ack(vh_packet_t *packet)
{
    ...
    u8 remote = (pkt_feature & 0x08) ? 1 : 0;
  ...
  if(bt_tws_is_connected()){
   bt_tws_disconnect();
        ...
        nor_link_info = bt_nor_get_link_info(NULL);
        if (tws_role) {
            if (nor_link_info && !remote) {
                if(!bt_get_qr_addr(new_addr)){
                    memcpy(new_addr, xcfg_cb.bt_addr, 6);          //本地的地址
                }
                tws_role = 0;
            } else {
...
u8 woerma_cmd_rsp(vh_packet_t *packet, u8 len, u8 result)
{
    packet->header = 0xAA55;
    packet->distinguish = VHOUSE_DISTINGUISH;
    packet->length = len + 1;
    ...
    vhouse_cmd_ack(packet);
	printf("woerma_cmd_rsp      ");
	print_r(packet,packet->length + 10);
    return 0;
}

#if LED_IS_BLACK			//裴工修改灯暗问题，充满电出仓在入仓不回充
bool close_flag;
bool charge_dont_set_vddio(void)
{
	//return !close_flag;
	return true;				//不切VDDIO 电压，充电时功耗会大一点
}
...
void bsp_vhouse_analysis_packet(vh_packet_t *packet)
{
    ...
    switch (packet->cmd) {
		...
		case VHOUSE_CMD_WOERMA_CHARGE_BOX_VERSION:	//充电仓软硬件版本指令
            TRACE("VHOUSE_CMD_WOERMA_CHARGE_BOX_VERSION\n");
			woerma_cmd_rsp(packet,1,0);
			break;
		case VHOUSE_CMD_WOERMA_CHARGE_BOX_VBAT:		//充电仓电量指令
			TRACE("VHOUSE_CMD_WOERMA_CHARGE_BOX_VBAT\n");
			woerma_cmd_rsp(packet,1,0);
			break;
		case VHOUSE_CMD_WOERMA_PAIR:			//长按3S清除配对指令
			TRACE("VHOUSE_CMD_WOERMA_PAIR\n");
            woerma_cmd_rsp(packet,1,0);
			msg_enqueue(EVT_ENTER_PAIR_MODE);
			break;
		case VHOUSE_CMD_WOERMA_INBOX:			//充电仓从外部入仓指令
			TRACE("VHOUSE_CMD_WOERMA_INBOX\n");
			woerma_cmd_rsp(packet,1,0);
			break;
#else
        case VHOUSE_CMD_GET_VBAT:
        ...
        if(huart_rx_buf[4]<VH_DATA_LEN){
	        ...
	        if(packet->checksum == crc8_maxim((u8 *)packet, 5 + packet->length)){
				...
				if(packet->cmd == 8)
				{
					sys_cb.pwr_flag = 0;
					if(param_inbox_read()){
						param_inbox_write(0);		//仓内关机
						
						printf("close window 8 write 0\n");
					}
				}
	            if (charge_sta) {
