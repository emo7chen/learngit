...
AT(.com_text.tws_sync)
void bsp_tws_sync_sco(u32 param1, u32 param2)
{
    sys_cb.sco_wait_timer = param1;
}
...
void bsp_tws_set_led(const void *cfg)
{
    if(bt_tws_is_connected()) {
        if(!bt_tws_is_slave()) {
			...
			if(sys_cb.slaver_flush_led_flag)		//优化同步灯
			{
				sys_cb.slaver_flush_led_flag = 0;
				led_set_sta_normal(cfg);		//  --- 为了连接手机时，从机可以自己执行灯显
			}
#else
...
{
#if USER_LED_SYSN_EN
	//printf("tws_remote_set_ledsta \n");
	//if(!sys_cb.inbox_led_flag){
		led_set_sync_sta(buf);
	//}
...
#if CONNECT_MP3_EN
//	if (bsp_tws_sync_res.res.num == TWS_RES_CONNECTED && (sco_is_connected() ||(!a2dp_is_silence(1) || !a2dp_is_silence(0)))) {
//		bsp_tws_sync_res.res.param1 = 0;										//RES_TYPE_INVALID
//		bsp_tws_sync_res.res_type = 0;
//		bsp_tws_sync_res.res_num = 0;
//		return;
	    if (bsp_tws_sync_res.res.num == TWS_RES_CONNECTED) {//这里去掉其他条件
        {
            static u32 tick_timeout = 0;
            if (0 == tick_timeout) {
                tick_timeout = tick_get();
                return;
            } else if(tick_check_expire(tick_timeout, 2000)){
                tick_timeout = 0;
            } else if(sco_is_connected() ||(!a2dp_is_silence(1) || !a2dp_is_silence(0))){//这里加拦截条件
                bsp_tws_sync_res.res.param1 = 0;                                        //RES_TYPE_INVALID
                bsp_tws_sync_res.res_type = 0;
                bsp_tws_sync_res.res_num = 0;
                tick_timeout = 0;
