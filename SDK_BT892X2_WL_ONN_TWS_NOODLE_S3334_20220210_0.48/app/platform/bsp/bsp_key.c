...
//    }
	if (xcfg_cb.ntc_en) {
	   get_ntc_adc_val((u16)(adc_cb.sfr[ADCCH_NTC]));
   	}
...
#if VBAT_TRIM_EN
	static u32 vbat_tick = 0;
	//static u8 vbat_count = 0;
#endif
...
#if VBAT_TRIM_EN
	if(CHARGE_DC_IN()){
		if(tick_check_expire(vbat_tick,2000))
		{
			sys_cb.vbat_tirm = 0;
		}
	}

	if(sys_cb.vbat_tirm > adc_cb.vbat_val){
		sys_cb.vbat_tirm = adc_cb.vbat_val;
	}

	if(sys_cb.vbat_tirm){
		vbat_bak = sys_cb.vbat_tirm;
	}else{
...
static void power_on_check_do(void)
{
...
#if 0//CHUANYUN_MODE			//修改船运模式，第一次开盖不开机的问题
		//chbox_out2pwr_en = xcfg_cb.ch_out_auto_pwron_en;
#endif
    ...
    while (1) {
        ...
        if (xcfg_cb.charge_en) {
#if CELAR_INFO_ID_EN
			clearinfo_id = param_clearinfo_read();
			if(clearinfo_id == 1)
			{
				printf("clear info retrun \n");
				if(bt_tws_get_link_info(NULL))
				{
					printf("don't clear info -------------------------------------------------\n");
					bt_tws_delete_link_info();
				}
				param_clearinfo_write(0);
				sys_cb.from_box_power_on = 1;
				//return;
				break;
			}
...
u8 bsp_key_scan(void)
{
	...
	static u8 press_flag1 = 0;
	static u8 press_flag2 = 0;
#endif
...
#if 0//USER_INEAR_DETECT_EN			//出耳 触摸失效
...
#if QUERY_VERSION_EN
		if(key == KH_PLAY_PWR_USER_DEF)
		{
			press_cnt++;
			//printf("press_cnt == %d\n",press_cnt);
		}

		if(press_cnt == 23 && !press_flag1){
			press_flag1 = 1;
			msg_enqueue(EVT_LONGPRESS_5S); //长按5S事件
		}
		if(press_cnt == 6 && !press_flag)
		{
			press_flag = 1;
			msg_enqueue(EVT_LONGPRESS_1_5S);		//长按1.5S事件
		}

		if(press_cnt == 14&& !press_flag2)
		{
			press_flag2 = 1;
			msg_enqueue(EVT_LONGPRESS_3S);		//长按3S事件
		}
		if(key == KU_PLAY_PWR_USER_DEF || key == KLU_PLAY_PWR_USER_DEF || key == KL_PLAY_PWR_USER_DEF){
				press_cnt = 0;
				press_flag = 0;
				press_flag1 = 0;
				press_flag2 = 0;
