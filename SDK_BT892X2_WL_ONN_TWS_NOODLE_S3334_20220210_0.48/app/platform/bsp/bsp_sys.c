...
void pwrkey_5s_on_check(void)
{
    ...
    if (sys_cb.pwrkey_5s_check) {
        if (IS_PWRKEY_PRESS() || IS_TKEY_PRESS()) {
            if (tick_check_expire(sys_cb.ms_ticks, 1000 * (xcfg_cb.bt_pwrkey_nsec_discover+1))) {
...
void usr_tmr1ms_isr(void)
{
    ...
    if (sys_cb.sco_wait_timer) {
        if (--sys_cb.sco_wait_timer == 0) {
             msg_enqueue(EVT_SYNC_SCO_ON);
        }
    }

    if (sys_cb.sco_wait_timeout) {
        if (--sys_cb.sco_wait_timeout == 0) {
             msg_enqueue(EVT_SYNC_SCO_ON);
        }
    }
    ...
    if(tick % 50 == 0) {
#if POWER_UP_LED_CTL
		if( (sys_cb.pwr_led_on_ctl!=1) && (sys_cb.charge_ok_led_flag!=1) )
		{
        	led_scan(tick);
		}
#else
...
void charge_inbox_check(void)
{
		if(CHARGE_INBOX()){
			sys_cb.outbox_cnt = 0;
			sys_cb.inbox_cnt++;
			sys_cb.outbox_siri_cnt = 3;
			if(sys_cb.inbox_cnt> 254)
			{
				sys_cb.inbox_cnt = 100;
			}
			if(sys_cb.inbox_cnt ==2)
			{
				sys_cb.inbox_led_flag = 1;
				if(bt_tws_is_connected() && !bt_tws_is_slave())
				{
					msg_enqueue(EVT_TWS_SWITCH);			//主机入仓，需要执行主从切换，并静音
				}
				else
				{
					msg_enqueue(EVT_FADE_IN);				//从机或单耳直接静音
				}
			}
		}
		else
		{
			...
			sys_cb.outbox_cnt++;
			if(sys_cb.outbox_cnt == 2)
			{
#if VBAT_TRIM_EN
				sys_cb.vbat_tirm = 0xFFFF;
#endif
				sys_cb.charge_led_cnt = 0;
				sys_cb.from_box_power_on = 0;
				sys_cb.into_pair_flag = 0;   //长按3S仓，进入配对的话，需要把红灯常亮刷新成配对灯
			}
			if(sys_cb.outbox_cnt> 254)  //如果一直在仓外，防止255溢出
			{
				sys_cb.outbox_cnt = 100;
			}
			if(sys_cb.inbox_cnt){
				if(sys_cb.outbox_cnt == 2)
				{
					sys_cb.inbox_led_flag = 0;
					sys_cb.from_box_power_on = 0;   //出仓过，标志至0
					msg_enqueue(EVT_FADE_OUT);
					sys_cb.inbox_cnt = 0;
				}
			}
		}
...
void charge_inbox_check(void)
{
	static u8 inbox_cnt = 0;
	static u8 outbox_cnt = 0;
		if(CHARGE_INBOX()){
			outbox_cnt = 0;
			inbox_cnt++;
			if(inbox_cnt> 254)
		...
		else
		{
			...
			if(outbox_cnt> 254)  //如果一直在仓外，防止255溢出
			{
				outbox_cnt = 100;
			}
			if(inbox_cnt){
...
void usr_tmr5ms_thread(void)
{
	...
	if (dev_is_online(DEV_EARIN)) {
			sys_cb.outear_cnt = 0;
			if(sys_cb.ruer_cnt < RUER_71_TIME){
				sys_cb.ruer_cnt++;
			}
			if(sys_cb.ruer_cnt >=RUER_71_TIME){    // 272ms
				sys_cb.ruer_cnt = RUER_100_TIME;
			}
	}
	else
	{
			sys_cb.ruer_cnt = 0;
			if(sys_cb.outear_cnt < RUER_71_TIME){
				sys_cb.outear_cnt++;
			}
			if(sys_cb.outear_cnt >= RUER_71_TIME){
				sys_cb.outear_cnt = RUER_100_TIME;
			}
	}
#endif
    //5ms timer process
#if WL_LINKLOSS_EN
		 if(sys_cb.wait_linkloss_cnt >0)		 //linkloss的消息会比连接消息晚发等待100ms，切换灯效加提示音
		{
			sys_cb.wait_linkloss_cnt++;
			if(sys_cb.wait_linkloss_cnt>20)
			{
				sys_cb.wait_linkloss_cnt=0;
				if(!sys_cb.link_loss_flag)
				{
#if DISCONN_PAIR_CLOSE_MP3
					sys_cb.is_need_pairing = 0;	// 已经报了提示音
					sys_cb.pairing_two_blem= 1;	//单耳断链时，进入scanning状态会再次播报pairing
#endif
#if MP3_PWRON_FIX_EN
					sys_cb.tws_conn_flg = 3;
#endif
...
#if CHARGE_INBOX_CHECK
					//入仓不复位，检测到开盖标志位，用于边充电边使用得功能
					if(!sys_cb.from_box_power_on)
					{
					}
					else
					{
...
#endif // BT_PWRKEY_5S_DISCOVER_EN
#if USER_INEAR_DETECT_EN
			if(sys_cb.private_call_switch_time)
			{
				sys_cb.private_call_switch_time--;
				if(!sys_cb.private_call_switch_time)
				{
					msg_enqueue(EVT_CALL_PRIVATE_SWITCH);
				}
			}
#endif
    ...
    if ((tmr5ms_cnt % 20) == 0) {
        lowpwr_tout_ticks();
#if	FACTROY_TEST_EN							  //产测指令
	if(sys_cb.wlink_factory_pwroff_cmd) 	  //产测关机指令
		{
			sys_cb.wlink_factory_pwroff_cmd --;
			if(!sys_cb.wlink_factory_pwroff_cmd)	//当检测到耳机已经出仓一定时间进入关机状态
				{
					sys_cb.discon_reason = 0;	//不同步关机
					sys_cb.pwrdwn_tone_en = 1;
					func_cb.sta = FUNC_PWROFF;	//关机状态
				}
		}
...
#if CHARGE_INBOX_CHECK
		//if(bt_get_disp_status()>=BT_STA_CONNECTED)
		//charge_inbox_check();		//充电仓入仓检测
#endif

#if CONNECT_TWS_1S_LED
		if(sys_cb.tws_conn_1s_cnt >= 1)
		{
			sys_cb.tws_conn_1s_cnt++;

			if(sys_cb.tws_conn_1s_cnt >= 16)
			{
				sys_cb.tws_conn_1s_cnt = 0;
				sys_cb.close_led = 0;
		...
		if(sys_cb.connect_flush_cnt >= 1)
		{
			sys_cb.connect_flush_cnt++;

			if(sys_cb.connect_flush_cnt >= 16)
			{
				sys_cb.connect_flush_cnt = 0;
				sys_cb.close_led = 0;
				msg_enqueue(EVT_BT_UPDATE_STA); 				//刷新显示
			}
		}
...
#endif // FUNC_CLOCK_EN
		msg_enqueue(MSG_SYS_500MS);
		if(sys_cb.dc_in_conut>0)
		{
			sys_cb.dc_in_conut --;
			if(!sys_cb.dc_in_conut)
			{
...
#if LOWPWR_MP3_EN
		if (sys_cb.vbat < ((u16)LOW_BATERRY_VBAT)) {			//低电压自定义
	...
	if(sys_cb.pwr_off_timeout_cnt && !bt_tws_is_slave() && !bt_nor_is_connected())
	{
		sys_cb.pwr_off_timeout_cnt --;
		if(!sys_cb.pwr_off_timeout_cnt)
		{
			//msg_enqueue(EVT_ENTER_SLEEP_MODE);
			msg_enqueue(EVT_300_PWR_OFF);
		}
	}
	if(sys_cb.pwr_off_timeout_cnt && bt_tws_is_slave()&& !bt_nor_is_connected())
	{
...
#endif
    //key_init();
   // charge_stop(0);      //修改开机电流过大的问题
   set_buck_mode(BUCK_MODE_EN);
...
#endif // FUNC_AUX_EN
            {
            	if(func_cb.sta!= FUNC_PWROFF && func_cb.sta != FUNC_BT_FCC){
                	func_cb.sta = FUNC_BT;
            	}
            }
        }
    ...
    if(!xcfg_cb.charge_en && xcfg_cb.charge_dc_reset){
        bsp_charge_dis();
    }
...
#endif
#if GT_TEST_EN				//指南测控
    sys_cb.fw_local = FW_VERSION;
    sys_cb.hw_local = HW_VERSION_INT;
#endif
#if DISCONN_PAIR_CLOSE_MP3
