...
#if RING_PRO_BLEM
	if (addr == RES_BUF_RING_MP3 && bt_get_disp_status() != BT_STA_INCOMING) {
		  break;
	}
#endif
...
#if POWER_UP_LED_CTL
		if(addr == RES_BUF_POWERON_MP3 && sys_cb.pwr_led_on_time_flag == 1)
		{
			...
			sys_cb.pwr_led_on_time_flag = 0;
			ticks = tick_get();
		}

		if(sys_cb.pwr_led_on_ctl == 1)
		{
			if(tick_check_expire(ticks, 1000))
			{
				sys_cb.pwr_led_on_time_flag = 0;
				sys_cb.pwr_led_on_ctl = 0;
				if (sys_cb.vbat < ((u16)LOW_BATERRY_VBAT)) {			//低电压自定义
					LED_PWR_SET_OFF();
				}
				else
...
u32 dac_src1_obuf[300]AT(.wavbuf.dec);
u32 *cfg_dac_src1_obuf = dac_src1_obuf;
u16 cfg_dac_obuf1_size = 300;
...
//        break;
    ...
    case TWS_RES_CALL_REJECT:
        ...
        *len = RES_LEN_CALL_REJECT_MP3;
		...
		case TWS_RES_TWS_CONN:
			*addr = RES_BUF_TWS_CONN_MP3;
			*len = RES_LEN_TWS_CONN_MP3;
			break;
#endif		// TWS_CONNECT_MP3_EN

	case TWS_LOW_BAT:
			*addr = RES_BUF_LOW_BATTERY_WAV;
			*len = RES_LEN_LOW_BATTERY_WAV;
		break;
