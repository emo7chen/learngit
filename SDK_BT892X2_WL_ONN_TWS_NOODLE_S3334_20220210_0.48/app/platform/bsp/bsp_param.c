...
#if CELAR_INFO_ID_EN
AT(.text.bsp.param)
u8 param_clearinfo_read(void)
{
    cm_read((u8 *)&sys_cb.clear_info_id, PARAM_CLEAR_INFO_FLAG, 1);
    return sys_cb.clear_info_id;
}

AT(.text.bsp.param)
void param_clearinfo_write(u8 sta)
{
    sys_cb.clear_info_id = sta;
    cm_write((u8 *)&sys_cb.clear_info_id, PARAM_CLEAR_INFO_FLAG, 1);
    cm_sync();
}
#endif
...
AT(.text.bsp.param)   
u8 param_inbox_read(void)     //sys_cb.pwr_flag = 1时，表示仓外关机，0表示仓内关机
{
    cm_read((u8 *)&sys_cb.pwr_flag, PARMAM_PWR_FLAG, 1);
    return sys_cb.pwr_flag;
}

AT(.text.bsp.param)
void param_inbox_write(u8 sta)
{
    sys_cb.pwr_flag = sta;
    cm_write((u8 *)&sys_cb.pwr_flag, PARMAM_PWR_FLAG, 1);
    cm_sync();
}
#endif
