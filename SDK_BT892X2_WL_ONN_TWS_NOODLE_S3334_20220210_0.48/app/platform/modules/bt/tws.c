...
uint8_t  cfg_bt_nor_connect_times           = 7;    //设置上电回连手机次数, 1.28s * n, 不小于5次
uint8_t  cfg_bt_tws_search_slave_times      = 5;    //设置TWS搜索次数, 1.25s * n, 不小于2次
uint8_t  cfg_bt_tws_connect_times           = 7;    //设置上电回连TWS次数, 1.28s * n, 不小于4次
uint16_t cfg_bt_tws_sup_to_connect_times    = -1;  //设置TWS断线回连次数, (1.28s + 1) * n, 设置(-1)为一直回连
...
void bt_tws_set_info(uint8_t *param)
{
    ...
    tmp = flag;      //避免编译警告
#if GT_TEST_EN					//测控指南
	param[3] =	sys_cb.fw_local;
	param[4] =	sys_cb.hw_local;
#endif
